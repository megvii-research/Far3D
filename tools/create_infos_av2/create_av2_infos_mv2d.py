...
def create_av2_infos(dataset_dir, split, out_dir):
    ...
    for i in tqdm(range(len(sensor_caches))): 
        # if i % 1000 != 0: # to create mini pkl for debug
        #     continue
        infos = {}
        ...
        if split != 'test':
            ...
            gt3d_infos, gt_bboxes_3d, names, gt_labels_3d = get_gt3d_data(dataset_dir=dataset_dir, split=split, log_id=log_id, 
            ...
            gt2d_infos = get_gt2d_data(camera_models, cam_infos, timestamp_city_SE3_ego_dict, lidar_timestamp_ns, 
                                       gt_bboxes_3d, names, gt_labels_3d)
    ...
    info_path = osp.join(out_dir, 'av2_{}_infos_mv2d_2.pkl'.format(split))
...
def get_gt3d_data(dataset_dir, split, log_id, timestamp_ns):
    ...
    gt_labels_3d = np.array(gt_labels_3d, dtype=np.int32)
    mask = gt_labels_3d > -1
    ...
    return gt3d_infos, gt_bboxes_3d, names, gt_labels_3d

def get_gt2d_data(synchronized_imagery, cam_infos, timestamp_city_SE3_ego_dict, lidar_timestamp_ns, annotations, names, gt_labels_3d):
    cam_name_to_img = {}
    if synchronized_imagery is not None:    
        ...
        for _, cam in synchronized_imagery.items():
            ...
            depths = []
            # gt_2dbboxes_sensor = []
            gt_bboxes_2d_to_3d_sensor = []
            match_idx = np.arange(len(annotations))
            ...
            centers3d = annotations.gravity_center.numpy() # N, 3
            categories_idx =np.array([label_class
                             for label_class in names])
            cuboids_vertices_ego = annotations.corners.numpy() # 8 corners of annotations
            ...
            for i in range(cuboids_vertices_cam.shape[0]):
                ...
                gt_2dlabels.append(CLASSES.index(categories_idx[i]))
                centers2d.append(uv[i])
                depths.append(centers3d_cam[i, 2])
                # gt_2dbboxes_sensor.append(centers3d_cam[i, :3])
                gt_bboxes_2d_to_3d_sensor.append(match_idx[i])
            
            gt_2dbboxes = np.array(gt_2dbboxes, dtype=np.float32)
            gt_2dlabels = np.array(gt_2dlabels, dtype=np.int32)
            centers2d = np.array(centers2d, dtype=np.float32)
            depths = np.array(depths, dtype=np.float32)
            gt_bboxes_2d_to_3d_sensor = np.array(gt_bboxes_2d_to_3d_sensor, dtype=np.int32)
            # gt_2dbboxes_sensor = np.array(gt_2dbboxes_sensor, dtype=np.float32)
            
            if not (gt_2dlabels[gt_bboxes_2d_to_3d_sensor > -1] == gt_labels_3d[gt_bboxes_2d_to_3d_sensor[gt_bboxes_2d_to_3d_sensor > -1]]).all():
                print(lidar_timestamp_ns)
            ...
            # gt_2dbboxes_sensors.append(gt_2dbboxes_sensor)
            gt_bboxes_2d_to_3d.append(gt_bboxes_2d_to_3d_sensor)
            ...
            # cam_name_to_img['gt_2dbboxes_cam'] = gt_2dbboxes_sensors
            cam_name_to_img['gt_bboxes_2d_to_3d'] = gt_bboxes_2d_to_3d
